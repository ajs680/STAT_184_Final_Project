...
```
Our top 10 white population counties list in the US includes 4 counties from Kentucky, and 3 counties in West Virgina. Although the group average is far lower than the national average, it is interesting to point out that all of these states are in a neighboring vicinity to one another. 
...
Top10Black %>%
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The average for top 10 black population counties in the US exactly matches the national average. This is the only demographic with an average this high. A whopping 7 out of 10 of these counties are from the state of Mississippi, which is located right next to Alabama, the state of our top 2 counties in this list. The geopgraphic location of these state is not coincidense. 
...
Top10Hispanic %>%
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The average percent of cases in the hispanic population is well under the national average. From inspecting our list, we see that ALL of the top 10 hispanic counties in the US is from Texas. 
...
Top10Asian %>%
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The average infection rate for asian populations is 0.18%, closer to the national average than other groups are. Middlesex, New Jersey has the highest infection rate we have seen so far at 1.15%, over 4 times the national average. This list contains 5 counties from California and 3 from Hawaii. 
...
Top10NA %>%
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The average infection rate for Native Americans is about half the national average; although, this is still higher than our "Top10White" and "Top10Hispanic". The most popular states for this demographic is Alaska, North and South Dakota. Mckinley, New Mexico has an obscenly high rate of COVID-19 cases, reaching almost 80%. 
#### Native Hawaiian
```{r}
Top10NH %>%
  ggplot(aes(x = county, y = pctCases)) +
...
Top10Multi %>%
  ...
  theme(axis.text.x = element_text(angle = 35))
```
Adair, Oklahoma has almost reached the national average by having a 0.23% infection rate. However, the average infection rate for the multiracial demographic is stll considered to be low at 0.07%. Oklahoma and Hawaii make up almost the entire list.
...
Top10Child %>%
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The average COVID rate for children is very low. The outbreak in Adams, Washington has almost reached the national average. 
#### Young Adults
```{r}
Top10YA %>%
  ggplot(aes(x = county, y = pctCases)) +
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The young adault demographic has the lowest average contraction rate than any of the age or race groups. Within this spread, we have Harrisonburg City, Virginia with a Coronavirus rate of 0.63%. It would be benificial to further explore geographic causes to this outbreak. 
#### Middle-Aged
```{r}
Top10Middle %>%
  ggplot(aes(x = county, y = pctCases)) +
  ...
  theme(axis.text.x = element_text(angle = 35))
```
The group average for middle-aged people in the U.S. is just under half of the national average. The top rate in this list is Hunterdon, New Jersey. Given the high volume of outbreaks noted in New Jersey from the previous demographics, it is not surprising that this city had high outbreaks than others in different states. A majority of middle aged people live in Colorado as the top 7 counties in this list are from Colorado. 
#### Senior
```{r}
Top10Senior %>%
  ggplot(aes(x = county, y = pctCases)) +
...
The group average for senior citizens is low compared to the national average. Some cities in this demographic come close to 0.26% but the overall results are not abnormal. 

###Closing Thought Analysis

If we take a look at the bar graphs produced from the 7 racial groups, we can idenify a general trend. This trend is that all the top counties for a particular group have approximately the same outbreak levels with spikes in one or two counties. This, along with the fact that most of the group averages(with the exception of the black demographic) are below the national average, signifies that the COVID-19 virus does not primarily spread through racial groups wihtin an area. The most interesting graph that came from investigating these racial groups was the graph for black population. This was the only demographic that came close to the national average. Our second highest group average came from the asian demographic at 0.18%; however, this average was brought up by the unusally high number of cases in Middlesex, New Jersey. Besides the black population, the overal group averages for race were extremly low. Further analysis can be conducted the see how the outbreak came to spread disproportionally to black populations. 


Shifting gears to the 4 age groups of interest, we see more fluctuation in graph behavior. The children and young adult bar graphs follow similar behavior seen in the racial groups, i.e. low rates with large spikes in 1 or 2 counties. The middle aged group, probably the group comprised of the most essential workeres, have the highest group average across all age groups. Even though the senior population does not have the highest infectiopn rate, it seems to have the least variation across all the counties. This is starting to indicate how certain social interactions are contributing to the spread of this virus. Senior citizens are most likely to be placed in group housing, surrounded by more seniors, which could be the reason why this demographic has the most consistant numbers. Children and young adults, tend to be more limited in social interaction because many do not have jobs or are students. It would be worth exploring how the outbreak has spread geographically. In combination with this, our data can be more useful and help us answer more questions. 
